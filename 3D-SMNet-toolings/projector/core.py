...
def _transform3D(position, rotation, device=torch.device("cpu")):
    """ 
    ...
    T = torch.zeros(1, 4, 4, device=device)
    T[:,0:3, 0:3] = R
    T[:,0:3,3] = torch.Tensor(position)
    T[:,3,3] =  1
...
class ProjectorUtils():
    ...
    def compute_scaling_params(self, batch_size, image_height, image_width):
        ...
        x = torch.arange(start=0, end=image_width, device=self.device)
        x = x.unsqueeze(0)
        x = x.repeat((image_height, 1))
        x = x.unsqueeze(0)
        x = x.repeat((batch_size, 1, 1))
        x = x.float()

        y = torch.arange(start=0, end=image_height, device=self.device)
        y = y.unsqueeze(1)
        y = y.repeat((1, image_width))
        y = y.unsqueeze(0)
    ...
    def pixel_to_world_mapping(self, depth_img_array, T):
        ...
        xyz1_t = self.transform_y_up(xyz1_t)
